...
void handlepiperedirected(string input,int angleindex, bool isAppend)
{
    ...
    int tempcount=0;
    int fd_filew;
    int filenameStart=angleindex+2;
    if(isAppend)
        filenameStart++;
    char* filename = getToken(input,&filenameStart,input.size());
    if(isAppend)
    {
        fd_filew = open(filename, O_APPEND | O_WRONLY);
    }else
    {
        fd_filew = open(filename, O_CREAT| O_WRONLY | O_TRUNC, 0700);
    }
